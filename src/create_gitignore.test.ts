...
Deno.test('createGitIgnore returns template in dry-run mode', async () => {
   const result = await createGitIgnore({ dryRun: true })
...
Deno.test('createGitIgnore writes .gitignore file when not dry-run', async () => {
   ...
   const writeFileMock = async (path: string, text: string) => {
      writtenPath = path
      writtenText = text
   }

   const result = await createGitIgnore({ dryRun: false }, { writeFile: writeFileMock })
