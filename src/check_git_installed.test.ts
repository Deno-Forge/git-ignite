...
import { spy, assertSpyCallAsync, assertSpyCalls } from '@std/testing/mock'

Deno.test("checkGitInstalled throws helpful error if git is not installed", async () => {
   await assertRejects(
       () => checkGitInstalled({ run: async () => {throw new Deno.errors.NotFound("git not found");} }),
       GitError,
...
Deno.test("checkGitInstalled throws when unexpected error encountered", async () => {
   await assertRejects(
       () => checkGitInstalled({ run: async () => {throw new Error("unexpected error");}}),
       Error,
       "unexpected error",
   );
});
...
Deno.test("checkGitInstalled returns true if git is installed", async () => {
   ...
   const runSpy: runShellCommand = spy(async () => {return { success: true }})
